...
5. 이클립스 JSP 개발 환경 구축
   ...
   2) 프로젝트 명 입력 시 식별자 작성 규칙을 준수
      ...
      클라이언트 측에서 (대소문자 구분없이) 좀 더 편하게 상요할 수 있또록
   ...
   6) 톰캣 서버 시작(재시작)
      ※ 이 시점에서...
         포트번호 설정 및 확인 필요
         (오라클이 웹 상에서 port8080 을 사용하기 때문에 충돌 방지)
         『Project Explorer』 의 『Server』 디렉터리 노드를 확장하면
         『server.xml』 파일이 존재하며, 이 파일을 열어 63 ~ 65 정도의 라인 내용을 수정한다
   ...
   8) 톰캣 서버가 실행되고 있는 상태에서
      ...
      <http://서버주서>:포트번호/프로젝트이름/파일이름.jsp
...
○ JSP 구성 요소
   ...
   1. 디렉티브(지시어)
	  ...
	  - buffer : 출력 버퍼 크기 지정
 	  - autoFlush : 출력 버퍼가 다 채워졌을 경우
			자동으로 버퍼에 있는 데이터를 비우게 마들지의 여부 지정
	  - info : 페이지에 대한 설명
   ...
   2. 스크립트 요소
	...
	스크립트 릿이나 표현식에서 사용할 수 잇는 변수나 메소드를 정의하는
	부분이기 때문에 선언부에서 선언된 변수는 서블릿으로 변환되는 과정에서
	멤버 변수의 입장으 취하게 되며 전역 변수의 성격을 가진다.
